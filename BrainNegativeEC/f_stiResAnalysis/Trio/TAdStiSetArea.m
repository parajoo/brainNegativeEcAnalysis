    ...
    function TAdStiSetArea(set_area,name)
    load trioCityBest718.mat
    centers = cluster.C;
    iFC_all_per = reshape(iFC_all,[timepoints,nsub,n_areas,n_areas]);
    iFC_all_new = permute(iFC_all_per,[2,1,3,4]);
    iFC_all_cut =zeros(length(adidx),timepoints,n_areas,n_areas);
    ...
    load ('hc895_data.mat','hc895_simfc','hc895_simpms')
    pms_hcsim_best = hc895_simpms;
    load ('ad943_data.mat','ad942_ec')
    SC1 =ad942_ec;
    FCemp1_hcbest = mean(hc895_simfc,3);
    N = size(SC1,1);
    ...
    iter = 1;
    %% part6:EI - DMF model
    %load dsi_mink_ad.mat
    stimulus = -0.01:0.001:0.01;
    sti_areas = zeros(N,1);
    idx_area = set_area;
    %idx_area = mi';
    sti_areas(idx_area) = 1;
    ...
    for ii = 1:length(stimulus)
        ...
        results(ii).stimulus = stimulus(ii);
        parfor tr = 1:nTrials %并行
            ...
            for t=0:dt:(1000*(Tmax - 1)*TR)
                x = w*JN11*y + I0 + JN11*G_coupling_local*(Cnew*y)+stimulus(ii)*sti_areas;
                r = feval(H,x);
        ...
        results(ii).pms_ad_sim = pms_ad_sim;
        kld_pms = calculateKLDivergences_test_new(pms_hcsim_best,pms_ad_sim);
...
%%  save data
cd /home/zengmin/model/pms_tianjin_data/trio_scale_622/trioResCity_Sti0718/AD/
save(sprintf('%s.mat',name));
cd /home/zengmin/model/pms_tianjin_data/trio_scale_622/
% clear all
  end
