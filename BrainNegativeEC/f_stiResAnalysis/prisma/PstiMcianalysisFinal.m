%% p-mci-ne-CandD
    clear all
    close all
    load pStiMciNeElocRes.mat
    name = 'p_mcineCandD';
    sect = name;
    figure_name = name;
    load('mci874_data.mat','mci874_simfc','mci874_simpms')
    ...
    region_pad = {'tplEloc','atnEloc','ftlEloc','ptlEloc','haMs','atnMs','ftlMs','bgMs'};
    region_idx = [1,2,5,6];
%     idxAll.ptlEloc = parietal;
...
%     idxAll.bgMs = basal_ganglia;
% 
%     region_pad = {'ptlEloc','occEloc','ftlEloc','dmnEloc','ftlMs','smnMs','visMs','bgMs'};
%     region_idx = 1:8;
    ...
    for i = 1:length(idxRes)
        ...
        stipms = results(idxRes(i)).pms_mci_sim;
        %[~,hcEloc,~,adEloc,~,stiadEloc] = calFcCCandEloc(hc874_simfc,mci874_simfc,stiadfc,1);
        [hcArea8,adArea8,stiadArea8,phcad8,phcstiad8] = getAreaElocOrMs(hcEloc,adEloc,stiadEloc,hcfc,adfc,stiadfc,region_pad,idxAll);
        stiResPlot_pad(hcArea8,adArea8,stiadArea8,phcad8,phcstiad8,'','MCI',region_pad,region_idx,'regulation MCI-neCandD');
        %function_plotPms4('PMS',hc874_simpms,mci874_simpms,stipms,'MCI')
    end
