...
fprintf('-- AD --  \n')
diff_th_range = 0:0.005:0.1;
ad_diff_ratio = zeros(1,length(diff_th_range));
ad_po_ratio = zeros(1,length(diff_th_range));
ad_ne_ratio = zeros(1,length(diff_th_range));
ad_ne_inter = zeros(1,length(diff_th_range));
ad_ne_intra = zeros(1,length(diff_th_range));
ad_enhance_ratio = zeros(1,length(diff_th_range));
ad_weaken_ratio = zeros(1,length(diff_th_range));
%% calculate the distribution of negative links(inter-brain and intra-brain)
for i=1:length(diff_th_range)
    diff_th = diff_th_range(i);
    fprintf('calculate the distribution of links after thresholding(diff_th = %.4f) \n',diff_th)
    diff = EC_reordered-SC_reordered;
    diff_erzhi = diff;
    diff_erzhi(abs(diff_erzhi)<diff_th) = 0;
    ratio_differzhi = 100*sum(diff_erzhi~=0,'all')/8100;
    %
    ad_diff_ratio(i) =ratio_differzhi; 
    %
    fprintf('ratio of diff(th = %.4f ): %.2f%%\n', diff_th,ratio_differzhi);
    diff_erzhi(diff_erzhi~=0) = 1;
    EC_thnew = diff_erzhi.*EC_reordered;
    ...
    fprintf('(diff_th = %.4f )positive edges: %.2f%%\n', diff_th,percentage_positive_new);
    fprintf('(diff_th = %.4f )negative edges: %.2f%%\n', diff_th,percentage_negative_new);
    fprintf('(diff_th = %.4f )positive edges(intra): %.2f%%\n', diff_th,percentage_intra_positive_new);
    fprintf('(diff_th = %.4f )positive edges(inter): %.2f%%\n', diff_th,percentage_inter_positive_new);
    fprintf('(diff_th = %.4f)negative edges(intra): %.2f%%\n', diff_th,percentage_intra_negative_new);
    fprintf('(diff_th = %.4f)negative edges(inter): %.2f%%\n', diff_th,percentage_inter_negative_new);
    ...
    fprintf('(diff_th = %.4f)weakened links: %.2f%%\n', diff_th,weakened_links_all*100);
    fprintf('(diff_th = %.4f)enhanced links: %.2f%%\n', diff_th,enhanced_links_all*100);
...
diff_th = diff_th_range(2);
ec_new = ec;
sc_new = sc;
diff_new = ec_new - sc_new;
diff_new_erzhi = diff_new;
diff_new_erzhi(abs(diff_new_erzhi)<diff_th)= 0;
