function Clusters = leida_clusters_testOula(input,maxk)

% k-means,output cluster parameters(struct)
%
...
Kmeans_results=cell(1,8);

parfor k=3:maxk  
    disp(['Calculating for ' num2str(k) 'clusters'])
    [IDX, C, SUMD, D]=kmeans(X,k,'Distance','sqeuclidean','Replicates',50,'Display','final'); %,'Options',opt);   
...
distM_fcd=squareform(pdist(X,'seuclidean'));
...
% save('LEiDA_Clusters_hc_ad_cen','Clusters')