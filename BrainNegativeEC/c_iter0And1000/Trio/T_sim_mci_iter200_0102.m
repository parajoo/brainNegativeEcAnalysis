 %% part1:calculate PMS of emperical subject
   ...
   load trioCityBest718.mat
    ...
    load ('group3Sc.mat','trioMciSc')
    mci_sc = trioMciSc(1:90,1:90,:);
    SC = mean(mci_sc,3);
    SC1 =0.2* SC /max(max(SC)) ;
    load ('group3Fc0707.mat','trioMciFc0707')
    FCemp1 = mean(trioMciFc0707,3);
    N = size(SC1,1);
...
%% initialize parameters for iteration
    ...
    %for ii = 1:length(G_coupling)
        ...
        results(iter).pms_mci_sim = pms_mci_sim;
        kld_pms = calculateKLDivergences_test_new(pms_mci,pms_mci_sim);
...
%         end
    ...
    save('mciCity1000_0718.mat')
    save('mciCity000Res_0718.mat','results','-v7.3')