 %% part1:calculate PMS of emperical subject
    ...
    load ('group3Sc.mat','trioHcSc')
    HC_sc = trioHcSc(1:90,1:90,:);
    SC = mean(HC_sc,3);
    SC1 =0.2* SC /max(max(SC)) ;
    load ('group3Fc0707.mat','trioHcFc0707')
    FCemp1 = mean(trioHcFc0707,3);
    N = size(SC1,1);
...
%% initialize parameters for iteration
    ...
    %for ii = 1:length(G_coupling)
        ...
        results(iter).pms_hc_sim = pms_hc_sim;
...
%     end
    ...
    save('hcCity1000_0718.mat')
    save('hcCity1000Results_0718.mat','results','-v7.3')