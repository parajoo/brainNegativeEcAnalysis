 %% part1:calculate PMS of emperical subject
    ...
    load ('group3Sc.mat','trioAdSc')
    AD_sc = trioAdSc(1:90,1:90,:);
    AD_sc(:,:,14) = [];
    SC = mean(AD_sc,3);
    SC1 =0.2* SC /max(max(SC)) ;
    load ('group3Fc0707.mat','trioAdFc0707')
    FCemp1 = mean(trioAdFc0707,3);
    N = size(SC1,1);
...
%% initialize parameters for iteration
    ...
    min_pms_G = results(min_index).pms_ad_sim;
